...
const OwnerSaleInquiriesPage: React.FC = () => {
  const { token } = useAuth();
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${apiBase}/sales/inquiries/owner`, { headers });
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 flex items-center">
            <MessageSquare className="w-7 h-7 mr-2 text-primary-600"/> Sale Inquiries (My Properties)
          </h1>
          <p className="text-gray-600 mt-1">Manage negotiations for properties you own.</p>
        </div>
          ...
          {items.map(i => (
            <div key={i.id} className="bg-white rounded-xl shadow-card p-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <a href={`/sales/inquiries/${i.id}`} className="px-3 py-1.5 rounded bg-primary-600 text-white hover:bg-primary-700 text-sm">Open</a>
              </div>
            </div>
          ))}
        </div>
...
export default OwnerSaleInquiriesPage;
